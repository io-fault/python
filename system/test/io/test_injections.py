...
def test_sockets_io_retry(test):
	...
	try:
		...
		g1 = io.__ERRNO_RECEPTACLE__['port_input_sockets'] = error(4)

		ep = network.Endpoint.from_ip4(('127.0.0.1', 0))
		fd = network.service(ep)
		r = io.alloc_service(fd)
		r.acquire((IA * 100))
		J.acquire(r)

		rep = network.Endpoint.from_ip4(('127.0.0.1', r.endpoint().port))
		fd = network.connect(rep)
...
def test_sockets_io_nomem(test):
	...
	try:
		...
		g1 = io.__ERRNO_RECEPTACLE__['port_input_sockets'] = error(4, errno.ENOMEM)
		...
		cep = r.endpoint()
		xep = network.Endpoint.from_ip4(('127.0.0.1', cep.port))
		channels = io.alloc_octets(network.connect(xep))
		...
		cep = r.endpoint()
		cep = network.Endpoint.from_ip4(('127.0.0.1', cep.port))
		channels = io.alloc_octets(network.connect(cep))
		J.acquire(channels[0])
		J.acquire(channels[1])
